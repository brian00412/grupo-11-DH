...
const validaciones = [
    check('nombre').notEmpty().withMessage ('Tenes que poner tu nombre 🙂').bail().isLength({ min: 2 }).withMessage ('minimo 2 caracteres'),
    ...
    check('email').notEmpty().withMessage ('Tenes que poner tu gmail 📧').bail().isEmail().withMessage ('tiene que ser un gmail existente'),
...
userRouter.get('/api/users/:id', usersController.detail);
